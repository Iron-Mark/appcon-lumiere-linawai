...
| **Processing** — working behind the scenes | `working` | Pill rays + cream motion arcs rotate (~3s loop); arcs pulse ~240ms opacity with staggered delays. Face fixed (closer dots, no mouth). **Reduced motion** → still frame (arcs visible, no spin/pulse) | e.g. “Clarifying…” |
